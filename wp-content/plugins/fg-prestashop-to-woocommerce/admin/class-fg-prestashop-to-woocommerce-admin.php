...
SQL;
						...
						foreach ( $product['specific_prices'] as $specific_price ) {
							if ( !isset($product['id_product_attribute']) || empty($specific_price['id_product_attribute']) || ($product['id_product_attribute'] == $specific_price['id_product_attribute']) ) {
								if ( $specific_price['reduction_type'] == 'percentage' ) {
									$special_price *= (1 - $specific_price['reduction']); // Percentage reduction
								} else {
									$special_price -= $specific_price['reduction']; // Amount reduction
								}
							}
						}
					}
		...
		protected function get_specific_prices($product_id) {
			...
			if ( version_compare($this->prestashop_version, '1.4', '>=') ) {
				...
				$sql = "
					SELECT $product_attribute_column, p.reduction, p.reduction_type, p.`from`, p.`to`
		...
		private function get_products($limit=1000) {
			...
			if ( version_compare($this->prestashop_version, '1.5', '<') ) {
				...
				$sql = "
					SELECT p.id_product, p.id_supplier, p.id_manufacturer, p.on_sale, p.quantity, p.price, p.reference, p.ean13, p.supplier_reference, $location_field, $width_field, $height_field, $depth_field, p.weight, p.out_of_stock, p.active, $available_for_order_field, p.date_add AS date, pl.name, pl.link_rewrite AS slug, pl.description, pl.description_short, pl.meta_description, pl.meta_keywords, pl.meta_title
					$reduction_fields
				...
				// PrestaShop 1.5+
				$sql = "
					SELECT DISTINCT p.id_product, p.id_supplier, p.id_manufacturer, p.on_sale, 0 AS quantity, p.id_shop_default, p.price, p.reference, p.ean13, p.supplier_reference, $location_field, p.width, p.height, p.depth, p.weight, 0 AS out_of_stock, p.active, p.available_for_order, p.is_virtual, p.date_add AS date, pl.name, pl.link_rewrite AS slug, pl.description, pl.description_short, pl.meta_description, pl.meta_keywords, pl.meta_title
					FROM ${prefix}product p
